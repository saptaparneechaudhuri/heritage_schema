        ...
        if (!isset($_GET['position'])) {
          ...
          if (count($available_texts) > 0) {
            for ($i = 0; $i < count($available_texts); $i++) {
              ...
              if ($mool_shloka_flag == TRUE) {
                // When no parameters are given.
                $node = Node::load($available_texts[$i]->nid);
                $mool_shloka['content'] = $node->field_ramayanamtest_25238_text->value;
                // Collect the metadata for mool shloka.
                $metadata = $_GET['metadata'];
                if (isset($_GET['metadata']) && $metadata == 1) {
                  $metadata_mool_sholka = collect_metadata($available_texts[$i]->nid, 'field_ramayanamtest_25238_text', 'dv');
                  $mool_shloka['meta_data'] = json_decode($metadata_mool_sholka, TRUE);
                ...
                $text_info['nid'] = json_decode($available_texts[$i]->nid, TRUE);
                $text_info['title'] = $available_texts[$i]->title;
                $text_info['field_ramayanamtest_25238_text'] = $mool_shloka;
              }
          ...
          if (isset($_GET['position'])) {
            ...
            if ($langcode == 'en') {
              ...
              $entityid_mool = get_entityId($position, $textname, 'dv');
              $node_mool = Node::load($entityid_mool);
              $mool_shloka['content'] = $node_mool->field_ramayanamtest_25238_text->value;
              $metadata = $_GET['metadata'];
              if (isset($_GET['metadata']) && $metadata == 1) {
                $metadata_mool = collect_metadata($entityid_mool, 'field_ramayanamtest_25238_text', 'dv');
                $mool_shloka['meta_data'] = json_decode($metadata_mool, TRUE);
              ...
              $metadata = $_GET['metadata'];
              if (isset($_GET['metadata']) && $metadata == 1) {
                $metadata_mool = collect_metadata($entityid, 'field_ramayanamtest_25238_text', $langcode);
                $mool_shloka['meta_data'] = json_decode($metadata_mool, TRUE);
            ...
            if ($mool_shloka_flag == TRUE) {
              $contents['field_ramayanamtest_25238_text'] = $mool_shloka;
