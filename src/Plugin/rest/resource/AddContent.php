    ...
    if (count($arg) == 0) {
      ...
      for ($i = 0; $i < count($arg); $i++) {
        if (!isset($arg[$i]['sourceid'])) {
          ...
          $source_present = db_query("SELECT COUNT(*) FROM `heritage_source_info` WHERE id = :sourceid", [':sourceid' => $arg[$i]['sourceid']])->fetchField();
          if ($source_present == 1) {

            $field_name = 'field_' . 'gita' . '_' . $arg[$i]['sourceid'] . '_text';
